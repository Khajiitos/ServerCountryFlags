package me.khajiitos.servercountryflags.forge.screen;
...
public class ServerMapScreen extends Screen {
    ...
    public ServerMapScreen(Screen parent) {
        super(new TranslationTextComponent("servermap.title"));
        this.parent = parent;
    ...
    public void init() {
        this.addButton(new Button(
                ...
                new TranslationTextComponent("selectServer.refresh"),
                (button) -> {
                    this.buttons.clear();
                    this.children.clear();
                    this.init();
    ...
    public void render(MatrixStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        Widget.drawCenteredString(poseStack, this.font, this.getTitle(), this.width / 2, 12, 0xFFFFFFFF);
        Widget.fill(poseStack, 0, 32, this.width, this.height - 32, 0xAA000000);
    ...
    public class Point {
        ...
        public List<IReorderingProcessor> getTooltip() {
            ...
            for (NamedLocationInfo info : locationInfos) {
                ...
                if (info.name == null) {
                    list.add(new TranslationTextComponent("servermap.home").withStyle(TextFormatting.BOLD).getVisualOrderText());
                } else {
                    list.add(new StringTextComponent(info.name).withStyle(TextFormatting.BOLD).getVisualOrderText());
                    list.add(new StringTextComponent((Config.showDistrict && !info.locationInfo.districtName.equals("") ? (info.locationInfo.districtName + ", ") : "") + info.locationInfo.cityName + ", " + info.locationInfo.countryName).getVisualOrderText());
                }
            }
            return list;
        }

        private void render(MatrixStack poseStack, boolean hovered) {
